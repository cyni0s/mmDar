...
Output per trajectory:
  - data/processed/radar_{traj}.pt  -- (N, 8, 512) complex64 (normalized)
  - data/processed/lidar_{traj}.pt  -- (N, 8192, 3) float32
  - data/processed/norm_{traj}.pt   -- (N,) float32 normalization factors
...
def lidar_fps_fixed(pts_xyz: np.ndarray, n_pts: int = N_LIDAR_PTS) -> np.ndarray:
    ...
    if len(filtered) >= n_pts:
        ...
        result = np.asarray(fps_pcd.points, dtype=np.float32)
        # open3d FPS deduplicates internally — may return fewer than n_pts
        # if there are duplicate points in the input. Pad if needed.
        if len(result) < n_pts:
            n_reps = n_pts // len(result) + 1
            result = np.tile(result, (n_reps, 1))[:n_pts]
        return result.astype(np.float32)
    else:
        # Padding: tile and truncate
