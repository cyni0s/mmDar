...
Set epoch_num to -1 to load best.pt_gen (best checkpoint by training loss).
"""
...
params = {
    ...
    'expt': 1,
    'dt': '20220320-034822',
    'epoch_num': 120,   # Set to -1 to use best.pt_gen
    'data': 5,
    'gpu': 1,
}
...
def main():
    ...
    # Load checkpoint: epoch_num=-1 loads best.pt_gen
    if params['epoch_num'] == -1:
        model_file = os.path.join(LOG_DIR, 'best.pt_gen')
        epoch_str = 'best'
        print(f'Loading best checkpoint: {model_file}')
    else:
        epoch_str = '%03d' % params['epoch_num']
        model_file = LOG_DIR + epoch_str + '.pt_gen'
        print(f'Loading epoch checkpoint: {model_file}')
    ...
    for test_i, (test_data, test_label) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            im1_file_name = save_path + epoch_str + '_' + ordered_filename[test_i] + '_pred.png'
            im1.save(im1_file_name)

            label = np.squeeze(test_label.cpu().numpy())
            label = (label*255).astype(np.uint8)
            im1 = Image.fromarray(label)
            im1_file_name = save_path + epoch_str + '_' + ordered_filename[test_i] + '_label.png'
            im1.save(im1_file_name)
